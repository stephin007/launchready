...
import { useGetAdminStats, getGetAdminStatsQueryKey, useDeletePrd, useListPrds, type PrdSummary } from "@workspace/api-client-react";
...
type StatsShape = {
  ...
  prdsThisWeek: number;
  prdsPerDay: { date: string; count: number }[];
  priorityBreakdown: { P1: number; P2: number; P3: number };
  effortDistribution: { S: number; M: number; L: number; XL: number };
};

function useUserStats() {
  return useQuery<StatsShape>({
    queryKey: ["/api/prds/stats"],
    queryFn: async () => {
      const res = await fetch("/api/prds/stats", { credentials: "include" });
...
export default function AdminDashboard() {
  ...
  const { data: adminStats, isLoading: adminStatsLoading } = useGetAdminStats();
  const { data: userStats, isLoading: userStatsLoading } = useUserStats();
  const { data: adminPrds, isLoading: adminPrdsLoading } = useAdminListPrds();
  const { data: userPrds, isLoading: userPrdsLoading } = useListPrds();

  const deletePrd = useDeletePrd();

  const stats: StatsShape | undefined = isAdmin ? adminStats : userStats;
  const prds = isAdmin ? adminPrds : userPrds;
  const isLoadingStats = isAdmin ? adminStatsLoading : userStatsLoading;
  const isLoadingPrds = isAdmin ? adminPrdsLoading : userPrdsLoading;

  const handleDelete = (prdId: string) => {
    deletePrd.mutate({ id: prdId }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["/api/admin/prds"] });
        queryClient.invalidateQueries({ queryKey: ["/api/prds/stats"] });
  ...
  if (isLoadingStats || isLoadingPrds) {
    return (
      <Layout>
        <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh]">
          <Loader2 className="w-8 h-8 animate-spin text-[var(--accent)] mb-4" />
      ...
      <div className="flex-1 w-full max-w-6xl mx-auto px-6 py-8 space-y-8">
        <div>
          ...
          <p className="text-[var(--text-muted)] mt-1">
            {isAdmin
              ? "Overview of all generated PRDs and tasks across all users."
              : "Overview of your generated PRDs and tasks."}
          </p>
        </div>
